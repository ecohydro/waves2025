...
export async function runCompleteMigration(
  ...
  try {
    ...
    if (results.people) {
      console.log(
        `People: ${results.people.successful}/${results.people.total} migrated (${results.people.imageUploads} images)`,
      );
    }

    if (results.publications) {
      console.log(
        `Publications: ${results.publications.successful}/${results.publications.total} migrated (${results.publications.pdfUploads} PDFs, ${results.publications.authorLinksCreated} author links)`,
      );
    }

    if (results.news) {
      console.log(
        `News: ${results.news.successful}/${results.news.total} migrated (${results.news.imageUploads} images, ${results.news.relatedLinksCreated} related links)`,
      );
    }

    if (results.validation) {
      console.log(
        `Validation: ${results.validation.passed}/${results.validation.totalChecks} checks passed (${results.validation.warnings} warnings, ${results.validation.errors} errors)`,
      );
    }
