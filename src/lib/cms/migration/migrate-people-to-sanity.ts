 ...
 */
function determineUserGroup(tags: string[] = [], status?: string): 'current' | 'alumni' | 'collaborator' | 'visitor' {
 ...
 */
async function transformPersonToSanity(mdxData: MDXPersonData, content: string): Promise<SanityPersonDocument> {
  ...
  const sanityDoc: SanityPersonDocument = {
    ...
    seo: {
      ...
      metaDescription: mdxData.excerpt || `Learn about ${mdxData.title}, ${mdxData.role || 'researcher'} at WAVES Research Lab.`,
      keywords: [
        mdxData.title.split(' '),
 ...
 */
async function processPerson(filePath: string): Promise<{ success: boolean; error?: string; imageUploads?: number }> {
  try {
 ...
 */
export async function migratePeopleToSanity(options: {
  sourceDir?: string;
  dryRun?: boolean;
  maxFiles?: number;
  skipImages?: boolean;
