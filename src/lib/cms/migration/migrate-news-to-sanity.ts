...
function determineNewsCategory(categories: string[] = []): SanityNewsDocument['category'] {
  ...
  if (
    categoryStr.includes('award') ||
    categoryStr.includes('achievement') ||
    categoryStr.includes('fellowship')
  ) {
    return 'award';
  }

  if (
    categoryStr.includes('outreach') ||
    categoryStr.includes('education') ||
    categoryStr.includes('community')
  ) {
    return 'outreach';
  }
  ...
  if (
    ...
    categoryStr.includes('team') ||
    categoryStr.includes('member')
  ) {
    return 'lab-news';
  }
 ...
 */
async function transformNewsToSanity(
  mdxData: MDXNewsData,
  content: string,
): Promise<{
  ...
  const {
    ...
    relatedProjects,
    relatedPeople,
    linksCreated: relatedLinksCreated,
  ...
  const sanityDoc: SanityNewsDocument = {
    ...
    seo: {
      ...
      keywords: [mdxData.title.split(' '), ...(mdxData.tags || []), ...(mdxData.categories || [])]
        .flat()
        .filter(Boolean),
    },
    analytics: {
 ...
 */
export async function migrateNewsToSanity(
  options: {
    sourceDir?: string;
    dryRun?: boolean;
    maxFiles?: number;
