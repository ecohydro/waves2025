 ...
 */
function determinePublicationType(publication: MDXPublicationData): 'journal-article' | 'conference-paper' | 'book-chapter' | 'preprint' | 'thesis' | 'report' | 'book' | 'other' {
  const venue = publication.journal || publication.venue?.name || '';
  const type = publication.type || '';
  
  // Check for preprints
  if (venue.toLowerCase().includes('arxiv') || venue.toLowerCase().includes('biorxiv') || 
      venue.toLowerCase().includes('preprint') || publication.arxivId) {
    return 'preprint';
  }
  ...
  if (venue.toLowerCase().includes('conference') || venue.toLowerCase().includes('proceedings') ||
      publication.booktitle || type.toLowerCase().includes('conference')) {
 ...
 */
async function uploadPDFToSanity(pdfPath: string): Promise<{ _type: 'reference'; _ref: string } | null> {
  try {
 ...
 */
async function processAuthors(authorsData: MDXPublicationData['authors'], authorRelationships?: MDXPublicationData['authorRelationships']): Promise<{
  ...
  // Handle array format
    ...
    for (const author of authorsData) {
      ...
      const _relationship = authorRelationships?.find(rel => 
      ...
      authors.push({
        _key: uuidv4(),
        ...(personRef ? { person: { _type: 'reference', _ref: personRef } } : { name: author.name }),
 ...
 */
// eslint-disable-next-line @typescript-eslint/no-unused-vars
async function transformPublicationToSanity(mdxData: MDXPublicationData, _content: string): Promise<{
  ...
  // Process authors
  const { authors, linksCreated: authorLinks } = await processAuthors(mdxData.authors, mdxData.authorRelationships);
  ...
  const sanityDoc: SanityPublicationDocument = {
    ...
    seo: {
      ...
      metaDescription: mdxData.abstract?.substring(0, 160) || `Research publication by WAVES Lab: ${mdxData.title}`,
    },
  };

  // Clean up empty objects
  if (Object.keys(sanityDoc.venue || {}).filter(key => sanityDoc.venue![key as keyof typeof sanityDoc.venue]).length === 0) {
  ...
  if (Object.keys(sanityDoc.metrics || {}).filter(key => sanityDoc.metrics![key as keyof typeof sanityDoc.metrics] !== undefined).length === 0) {
...
async function processPublication(filePath: string): Promise<{
  ...
  try {
    ...
    const { document: sanityDoc, linksCreated, pdfUploaded } = await transformPublicationToSanity(mdxData, content);
 ...
 */
export async function migratePublicationsToSanity(options: {
  sourceDir?: string;
  dryRun?: boolean;
  maxFiles?: number;
  skipPDFs?: boolean;
