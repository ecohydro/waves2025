...
export default async function NewsPage() {
  ...
  const renderNewsCard = (article: News, featured = false) => (
    <Card key={article._id} className={`group hover:shadow-lg transition-all duration-300 ${featured ? 'md:col-span-2' : ''}`}>
      <CardContent className="p-0">
        <Link href={`/news/${article.slug.current}`} className="block">
          <div className={`${featured ? 'md:flex md:gap-6' : ''}`}>
            {/* Featured Image */}
            <div className={`${featured ? 'md:w-1/2' : ''} aspect-video overflow-hidden ${featured ? 'md:rounded-l-lg' : 'rounded-t-lg'} bg-gray-100`}>
              {article.featuredImage ? (
                ...
                <div className="w-full h-full bg-gradient-to-br from-wavesBlue/10 to-blue-100 flex items-center justify-center">
                  <svg className="w-16 h-16 text-wavesBlue/30" fill="currentColor" viewBox="0 0 24 24">
            ...
            <div className={`p-6 ${featured ? 'md:w-1/2 md:flex md:flex-col md:justify-between' : ''}`}>
              <div>
                ...
                {/* Title */}
                <h3 className={`font-semibold text-gray-900 group-hover:text-wavesBlue transition-colors mb-3 leading-snug ${featured ? 'text-xl' : 'text-lg'}`}>
                  {article.title}
                </h3>

                {/* Excerpt */}
                <p className={`text-gray-600 leading-relaxed mb-4 ${featured ? 'text-base line-clamp-4' : 'text-sm line-clamp-3'}`}>
                  {article.excerpt}
                </p>
          ...
          <div className="container max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {allCategories.map(category => {
                const categoryCount = allNews.filter(article => 
                ...
                return (
                  <Card key={category} className="group hover:shadow-md transition-shadow">
                    <CardContent className="p-6 text-center">
                      <div className={`inline-flex px-3 py-1 rounded-full text-sm font-medium mb-3 ${getCategoryColor(category)}`}>
