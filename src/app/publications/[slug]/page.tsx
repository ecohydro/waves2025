...
import { fetchPublicationBySlug, fetchPublications, urlForImage, type Publication } from '@/lib/cms/client';
...
export default async function PublicationDetail({ params }: PublicationDetailProps) {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'preprint': return 'bg-indigo-100 text-indigo-800';
        ...
        <div className="container max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-4xl mx-auto">
            ...
            <div className="flex flex-wrap gap-2 items-center mb-6">
              <span className={`inline-flex px-3 py-1 rounded-full text-sm font-medium ${getStatusColor(publication.status)}`}>
                {publication.status.charAt(0).toUpperCase() + publication.status.slice(1).replace('-', ' ')}
              </span>
            ...
            {publication.venue && (
              <div className="flex flex-wrap gap-4 text-lg text-gray-600 mb-8">
                <span className="font-medium text-wavesBlue">
                  {publication.venue.name}
                </span>
                {publication.venue.volume && (
                  <span>Vol. {publication.venue.volume}</span>
                )}
                {publication.venue.issue && (
                  <span>Issue {publication.venue.issue}</span>
                )}
                {publication.venue.pages && (
                  <span>pp. {publication.venue.pages}</span>
                )}
          ...
          <div className="max-w-4xl mx-auto">
            <div className="grid lg:grid-cols-3 gap-8">
                  ...
                  <CardContent className="p-8">
                    ...
                    <div className="bg-gray-50 p-4 rounded-lg">
                      <p className="text-sm text-gray-700 font-mono leading-relaxed">
                        ...
                        {publication.publishedDate && ` (${new Date(publication.publishedDate).getFullYear()})`}
              ...
              <div className="space-y-6">
                  ...
                  <CardContent className="p-6">
                    ...
                    <dl className="space-y-3">
                      <div>
                        ...
                        <dd className="text-sm text-gray-900">{getPublicationTypeLabel(publication.publicationType)}</dd>
                      </div>
                      
                      <div>
                        <dt className="text-sm font-medium text-gray-500">Status</dt>
                        <dd className="text-sm text-gray-900">{publication.status.charAt(0).toUpperCase() + publication.status.slice(1).replace('-', ' ')}</dd>
                      </div>
                      
                      {publication.publishedDate && (
                        <div>
                          <dt className="text-sm font-medium text-gray-500">Published</dt>
                          <dd className="text-sm text-gray-900">{formatDate(publication.publishedDate)}</dd>
                        </div>
                      )}
                      ...
                      {publication.submittedDate && (
                        <div>
                          ...
                          <dd className="text-sm text-gray-900">{formatDate(publication.submittedDate)}</dd>
                        </div>
                      )}
                      ...
                      {publication.acceptedDate && (
                        <div>
                          ...
                          <dd className="text-sm text-gray-900">{formatDate(publication.acceptedDate)}</dd>
                        </div>
                      )}
                ...
                {/* Metrics */}
                {publication.metrics && (publication.metrics.citations || publication.metrics.altmetricScore) && (
                  <Card>
                    <CardContent className="p-6">
                      ...
                      <dl className="space-y-3">
                        {publication.metrics.citations && (
                          <div>
                            ...
                            <dd className="text-2xl font-bold text-wavesBlue">{publication.metrics.citations}</dd>
                          </div>
                        )}
                        {publication.metrics.altmetricScore && (
                          <div>
                            <dt className="text-sm font-medium text-gray-500">Altmetric Score</dt>
                            <dd className="text-2xl font-bold text-wavesBlue">{publication.metrics.altmetricScore}</dd>
                          </div>
                        )}
                        {publication.metrics.impactFactor && (
                          <div>
                            <dt className="text-sm font-medium text-gray-500">Impact Factor</dt>
                            <dd className="text-lg font-semibold text-gray-900">{publication.metrics.impactFactor}</dd>
                          </div>
                        )}
                        {publication.metrics.quartile && (
                          <div>
                            <dt className="text-sm font-medium text-gray-500">Quartile</dt>
                            <dd className="text-lg font-semibold text-gray-900">{publication.metrics.quartile}</dd>
                          </div>
                        )}
                      </dl>
                    </CardContent>
                      ...
                      <div className="space-y-3">
                        {publication.links.publisher && (
                          ...
                          >
                            ...
                            <span className="text-sm text-gray-700 group-hover:text-gray-900">Publisher</span>
                          </a>
                        )}
                        ...
                        {publication.links.supplementary && (
                          ...
                          >
                            ...
                            <span className="text-sm text-gray-700 group-hover:text-gray-900">Supplementary Material</span>
                          </a>
                        )}
                      </div>
                    </CardContent>
